...
/*     $cible = unique($tab, 2);
    ...
    for ($i=0; $i < count($cible); $i++) {
        if (($cible[$i] != "Trinidad & Tobago")
        && ($cible[$i] != "Somaliland Region")) {
    ...
    for ($i=0; $i < count($tab); $i++) {
        ...
        if (isset($tab[$i+1][4])) {
            $cibleCSV = $db->prepare('SELECT id_val FROM vals WHERE val = :val');
            $cibleCSV->bindValue(':val', $tab[$i+1][4], PDO::PARAM_STR);
            $cibleCSV->execute();
            $resultHappyScore = $cibleCSV->fetch(PDO::FETCH_ASSOC);
            var_dump($resultHappyScore);
        }

        /* health score */
        if (isset($tab[$i+1][7])) {
            $cibleCSV = $db->prepare('SELECT id_val FROM vals WHERE val = :val');
            $cibleCSV->bindValue(':val', $tab[$i+1][7], PDO::PARAM_STR);
            $cibleCSV->execute();
