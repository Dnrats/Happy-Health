...
/*     $cible = unique($tab, 3);
 ...
 /* $cible = unique($tab, 11);
    ...
    /* Happiness Rank */
/*  $cible = unique($tab, 1);
    $query_happiness_rank = $db->prepare('INSERT INTO `values` (value) VALUES (:value)');
    for ($i=0; $i < count($cible); $i++) { 
        $query_happiness_rank->bindValue(':value', $cible[$i], PDO::PARAM_STR);
        $query_happiness_rank->execute();
    } */
    /* Happiness Score */
/*  $cible = unique($tab, 4);
    $query_happiness_score = $db->prepare('INSERT INTO `values` (value) VALUES (:value)');
    for ($i=0; $i < count($cible); $i++) { 
        $query_happiness_score->bindValue(':value', $cible[$i], PDO::PARAM_STR);
        $query_happiness_score->execute();
    } */
    /* Health Score */
 /* $cible = unique($tab, 7);
    $query_health_score = $db->prepare('INSERT INTO `values` (value) VALUES (:value)');
    for ($i=0; $i < count($cible); $i++) { 
        $query_health_score->bindValue(':value', $cible[$i], PDO::PARAM_STR);
...
/*     $cible = unique($tab, 2);
    ...
    for ($i=0; $i < count($tab); $i++) {
        ...
        $cibleCSV->bindValue(':name_country', isset($tab[$i+1][2]), PDO::PARAM_STR);
        $cibleCSV->execute();
        $resultCountry = $cibleCSV->fetch(PDO::FETCH_ASSOC);
        
        /* id years */
        $cibleCSV = $db->prepare('SELECT id_years FROM years WHERE `year` = :year');
        $cibleCSV->bindValue(':year', isset($tab[$i+1][11]), PDO::PARAM_INT);
        $cibleCSV->execute();
        $resultYear = $cibleCSV->fetch(PDO::FETCH_ASSOC);

        /* happiness rank */
        $cibleCSV = $db->prepare('SELECT id_values FROM `values` WHERE `value` = :value');
        $cibleCSV->bindValue(':value', isset($tab[$i+1][1]), PDO::PARAM_STR);
        $cibleCSV->execute();
        $resultHappyRank = $cibleCSV->fetch(PDO::FETCH_ASSOC);
        var_dump($resultHappyRank);
...
/*         $query_name_country = $db->prepare('INSERT INTO country_has_years (country_id, years_id, happiness_rank, happiness_score, health_score) 
        ...
        $query_name_country->bindValue(':happiness_rank', $resultHappyRank["id_values"], PDO::PARAM_INT);
        $query_name_country->bindValue(':happiness_score', $result["id_region"], PDO::PARAM_INT);
        $query_name_country->bindValue(':health_score', $result["id_region"], PDO::PARAM_INT);
        $query_name_country->execute(); */
    }
}
